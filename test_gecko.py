...
def test_retry_decorator_pass() -> None:
    ...
    __test_retry_decorator_base(number_of_exceptions_raised=3, number_of_retries=3, should_catch_error=False)
...
def test_retry_decorator_fail() -> None:
    ...
    __test_retry_decorator_base(number_of_exceptions_raised=4, number_of_retries=3, should_catch_error=True)


def __test_retry_decorator_base(number_of_exceptions_raised, number_of_retries, should_catch_error) -> None:
    ...
    try:
        decorated_function()
        assert(not should_catch_error)
    except FileNotFoundError:
        assert(should_catch_error)
